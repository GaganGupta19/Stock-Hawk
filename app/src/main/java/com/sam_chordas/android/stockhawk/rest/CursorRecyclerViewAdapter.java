...
public abstract class CursorRecyclerViewAdapter <VH extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<VH>{
  ...
  private static final String CURSOR_ERROR = "This should only be called when Cursor is valid";
  private static final String NEXT_VALUE_ERROR = "Could not move Cursor to position: ";
  ...
  public void onBindViewHolder(VH viewHolder, int position) {
    if (!dataIsValid){
      throw new IllegalStateException(CURSOR_ERROR);
    }
    if (!mCursor.moveToPosition(position)){
      throw new IllegalStateException(NEXT_VALUE_ERROR + position);
    }
